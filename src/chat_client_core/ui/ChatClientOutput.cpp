...
void ChatClientOutput::render_message(HumanChatMessage *message) {
    if (PublicChatMessage *public_message = dynamic_cast<PublicChatMessage *>(message)) {
        render_public_message(public_message);
    } else if (PrivateChatMessage *private_message = dynamic_cast<PrivateChatMessage *>(message)) {
        render_private_message(private_message);
    }
}

void ChatClientOutput::render_public_message(PublicChatMessage *message) {
    client_output("[" + time_to_str(message->get_time()) + "] " + message->sender_id() + ": " + message->body() + "\n");
}

void ChatClientOutput::render_private_message(PrivateChatMessage *message) {
